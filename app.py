...
st.set_page_config(page_title="Resume Screener Ã‚Â· LoRA fine-tuned", page_icon="??", layout="centered")
...
st.markdown(
    ...
    <div class="hero">
        ...
        <p>LoRA fine-tuned Qwen2.5-0.5B-Instruct Ã¢â‚¬â€ outputs structured JSON verdicts instead of prose,
...
with st.expander("How this model works"):
    st.markdown(
        """
        Base instruct models produce unstructured prose when asked to screen a resume Ã¢â‚¬â€
        ...
        text before display Ã¢â‚¬â€ any skill it claims that isn't really in the resume is
...
if run:
    ...
    if verdict.get("parse_error"):
        st.warning("Model output wasn't valid JSON Ã¢â‚¬â€ showing raw output:")
        ...
        years_display = years_display if years_display is not None else "Ã¢â‚¬â€"
